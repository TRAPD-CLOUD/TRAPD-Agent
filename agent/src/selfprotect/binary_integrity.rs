...
///   * the stored baseline does not match the running binary, OR
...
/// In that case integrity simply cannot be established yet; refusing to start
/// would block legitimate test deployments, so we degrade instead.
pub fn check() -> Result<()> {
    let exe = exe_path()?;
...
fn verify_ed25519_signature(hash_bytes: &[u8]) -> Result<()> {
    ...
    match (pubkey_path.exists(), sig_path.exists()) {
        (false, _) => {
            warn!(
                "Ed25519 public key not found at {} — \
        ...
        (true, false) => {
            warn!(
                "Ed25519 public key present but no signature found at {} — \
                 signature verification skipped.",
                sig_path.display()
            );
            return Ok(());
