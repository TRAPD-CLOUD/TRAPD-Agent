...
pub fn inspect_process(comm: &str, exe: &str, cmdline: &str) -> Option<DetectionData> {
    ...
    let clears_history = (lower.contains("history")
        && (lower.contains("-c") || lower.contains("/dev/null")))
    ...
    let pe = match base {
        ...
        "vim" | "vi" | "view"
            if lower.contains(":!") || lower.contains("-c") && lower.contains("sh") =>
        {
